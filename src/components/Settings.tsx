...
import { Trash2, UserPlus, Mail, Shield, Palette, CheckCircle2, Lock } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wider text-white">System Configuration</h2>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">Global Administrator Controls</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="theme-card p-8">
            <div className="flex items-center gap-6 mb-10">
              <div className="relative">
                {user.photoURL ? (
                  <img src={user.photoURL} alt="" className="w-20 h-20 rounded-2xl border-2 border-white/10 shadow-xl" referrerPolicy="no-referrer" />
                ) : (
                  <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center border-2 border-white/10">
                    <span className="text-2xl font-bold">{user.displayName?.[0] || user.email?.[0]}</span>
                  </div>
                )}
                <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-emerald-500 border-4 border-[#16161a] rounded-full"></div>
              </div>
              <div>
                <p className="text-xl font-bold text-white tracking-tight">{user.displayName || 'Operations Lead'}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-white/5 pt-8">
              <div className="space-y-3">
                <label className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em]">Regional Assignment</label>
                <div className="p-4 bg-[#111114] border border-white/5 rounded-xl text-xs text-slate-300 font-medium">
                  Phase 5 Drilling Operations Hub
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em]">Session ID</label>
                <div className="p-4 bg-[#111114] border border-white/5 rounded-xl text-xs text-slate-500 font-mono truncate">
                  {Math.random().toString(36).substring(7).toUpperCase()}-TERMINAL-SECURE
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="theme-card p-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-xs font-bold text-white uppercase tracking-widest">Interface Skin Selector</h3>
            </div>
              ...
              {themes.map((t) => (
                    ...
                    >
                      ...
                      <div className="flex gap-1 mb-3 relative z-10">
                        {t.colors.map((c, i) => (
                          <div key={i} className="w-3 h-3 rounded-full border border-white/10" style={{ backgroundColor: c }}></div>
                        ))}
                      </div>
                      <p className={cn(
                        "text-[10px] font-bold uppercase transition-colors relative z-10",
            ...
            <p className="text-[9px] text-slate-600 mt-6 uppercase italic font-mono tracking-tighter">
              * Global UI skin calibrated for high-visibility terminal operations
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="theme-card p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-xs font-bold text-white uppercase tracking-widest">Invite Personnel</h3>
            </div>
    <form onSubmit={handleInvite} className="space-y-4 relative">
      ...
      >
          ...
          <button 
            onClick={onLogout}
            className="w-full py-4 bg-[#111114] border border-white/10 text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] rounded-xl hover:text-white transition-all flex items-center justify-center gap-3"
          >
            Terminal Logout
          </button>
        </div>
